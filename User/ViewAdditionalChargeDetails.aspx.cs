    ...
    public void Bind()
    {
        ...
        if (type == "Join Account")
        {
            ...
            for (int i = 0; i < dsq.Tables[0].Rows.Count; i++)
            {
                ...
                string sql = "select * from AdditionalCharge_tb where UserId='" + dsq.Tables[0].Rows[i][0].ToString() + "'";
                DataSet dsqq = dm.For_Adapter(sql);
                if (dsqq.Tables[0].Rows.Count > 0)
                {
                    GridView1.DataSource = dsqq;
                    GridView1.DataBind();
                    lblmsg.Text = "";
                }
                else
        ...
        {
            string sql = "select * from AdditionalCharge_tb where UserId='" + Session["Id"].ToString() + "'";
            DataSet dsqq = dm.For_Adapter(sql);
            if (dsqq.Tables[0].Rows.Count > 0)
            {
                GridView1.DataSource = dsqq;

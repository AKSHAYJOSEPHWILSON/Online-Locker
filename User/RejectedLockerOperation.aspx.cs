                ...
                for (int i = 0; i < dsq.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataListItem it in DataList1.Items)
                        {
                            ...
                            string stre = "select * from OperationRequst_tb where UserId='" + dsq.Tables[0].Rows[i][0].ToString() + "' and Status='Rejected'";
                            ...
                            {
                                lblms.Text = "No Rejected operation";
                            }
            ...
            else
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataListItem it in DataList1.Items)
                    {
                        ...
                        Label lblms = (Label)it.FindControl("lblmessage");
                        string stre = "select * from OperationRequst_tb where UserId='" + Session["Id"].ToString() + "' and Status='Approved'";
                        DataSet dse = dm.For_Adapter(stre);
                        if (dse.Tables[0].Rows.Count > 0)
                        {

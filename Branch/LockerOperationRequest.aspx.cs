...
public partial class Branch_ViewLockerRequest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            string str = "select OperationRequst_tb.* from OperationRequst_tb inner join UserRegistration_tb on OperationRequst_tb.UserId=UserRegistration_tb.UserId where UserRegistration_tb.BranchId='" + Session["Id"] + "'and OperationRequst_tb.Status='Pending'";
            ...
            {
                lblmsg.Text = "No Pending Operation Request Found";
            }
...
lbllockerId.Text = GridView1.SelectedRow.Cells[2].Text;
    ...
    {
        string str = "Update OperationRequst_tb set Status='Approved' where OpRequestId='" + lblrequestid.Text + "'";
        int r = dm.For_Execute(str);
        if (r > 0)
        {
           
                Response.Write("<script language='javascript'>alert('Locker Request Approved Successfully...')</script>");
                Server.Transfer("~/Branch/LockerOperationRequest.aspx");
        }
    }
    protected void Button2_Click(object sender, EventArgs e)
    {
        string str = "Update OperationRequst_tb set Status='Rejected' where OpRequestId='" + lblrequestid.Text + "'";
        int r = dm.For_Execute(str);
        if (r > 0)
        {

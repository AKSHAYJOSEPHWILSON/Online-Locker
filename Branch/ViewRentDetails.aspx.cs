...
public partial class User_ViewRentDetails : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string type = dm.For_Scalar("select Type from UserRegistration_tb where UserId='" + Request.QueryString["UserId"] + "'");
            if (type == "Join Account")
            {
                string joinId = dm.For_Scalar("select JointAcId from JoinAccount_tb where UserId='" + Request.QueryString["UserId"] + "'");
            ...
            {
                Operations(Request.QueryString["UserId"]);
